    ...
    while (i < len(datos["results"])-1):
        ...
        try:
            ...
            cursor.execute(insert_query,(datos["results"][i]["login"]["uuid"],
                datos["results"][i]["login"]["username"],'$2b$10$FmOL5MxEi.v9rvigMYmAOOrMgDXxyCfLrUfDB9LSRy1G0O9DIjIae',datos["results"][i]["email"],datos["results"][i]["picture"]["large"],random.randrange(start=10),date_obj,\
                datos["results"][i]["location"]["street"]["name"]+','+str(datos["results"][i]["location"]["street"]["number"]),
                datos["results"][i]["name"]["title"],datos["results"][i]["name"]["first"],datos["results"][i]["name"]["last"],'28999','+(919) 367 788755',datos["results"][i]["cell"],datos["results"][i]["nat"],
