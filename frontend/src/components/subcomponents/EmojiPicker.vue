    ...
    <v-card  v-if="show">
      ...
      <v-card-text>
        <div class="emoji-content">
          <emoji v-for="emoji in emojis" v-bind:key="emoji.key" :emoji="emoji" @click_on_emoji="onEmojiClick"></emoji>
        </div>
      </v-card-text>
    </v-card>
  </div>
...
<script>
  ...
  export default {
    ...
    /*
    try {
      const response = await axios.get('https://raw.githubusercontent.com/shanraisshan/EmojiCodeSheet/master/json/string/People.json')  
    ...
    */
    ...
    methods: {
      onEmojiClick (emoji) {
        this.$emit('click_on_emoji', emoji)
      },
      onEmojiClose () {
        this.$emit('click_close_emoji')
      },
    },
    components: {
      'emoji': Emoji
